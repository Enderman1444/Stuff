...
bot.on('chat', (username, message, player) => {
  ...
  const command = message.split(' ')
     if (cmd === `${Prefix}help`){
      bot.chat(`(Owner Only): ${Prefix}say ${Prefix}kill ${Prefix}tpa ${Prefix}come (Everyone): ${Prefix}help ${Prefix}a ${Prefix}uuid ${Prefix}ping ${Prefix}tps ${Prefix}coords ${Prefix}equiparmor ${Prefix}craft ${Prefix}list ${Prefix}equip ${Prefix}unequip ${Prefix}use ${Prefix}fight me ${Prefix}guard ${Prefix}tossall  `)
     }
     if (cmd === `${Prefix}a`){
         bot.chat(`/tpyes`)
         bot.chat(`/tpaccept`)
  ...
  getRandomInt(3 * 60) * 1000);
      ...
      if (cmd === `${Prefix}equiparmor`) {
        ...
        bot.chat (`Equipped all best armor.`)
      }
      if (command[0] === `${Prefix}kill`){
        ...
        if (command[0] === `${Prefix}tossall`) {
          tossAll()
          bot.chat ("Throwing all items, ARMOR NOT INCLUDED!")
        }
  });
